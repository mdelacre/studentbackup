##################################################################################################################################
#                      BALANCED DESIGNS WITH  S1 = Chi(2) ; S2 = N_skewed(2,2*SDR,skewness=1.99)                                 #
##################################################################################################################################
...
setwd(dir="C:/Users/Admin/Documents/Github projects/studentbackup/scripts outputs/Appendices/Table A1/Table A1.9 One chi(2), one normal skewed")
...
###### n=10
	...
	alpha_bis=function(SDR){ 
		...
		generate=function(i){
			...
			sample1 <- rchisq(n,df=2,ncp=0) # mean = df = 2; sd = 2
			sample2 <- rsnorm(n, mean=2, sd=SDR*2,xi=-1.99) # mean = 2 (the same than in the chi square) because we want to test the type 1 error rate
	...
	write.table(alpha_rate,paste("Alpha extr SDR_balanced_chisq and equal skewed_n1=n2=",alpha_rate[1],".txt"),sep=";",dec=",")
...
###### n=10
	...
	power_bis=function(SDR){ 
		...
		generate=function(i){
			...
			sample1 <- rchisq(n,df=2,ncp=0) # mean = df = 2; sd = 2
			sample2 <- rsnorm(n, mean=3, sd=SDR*2,xi=-1.99) # mean = 3
	...
	write.table(power_rate,paste("Power extr SDR_balanced_chisq and equal skewed_n1=n2=",power_rate[1],".txt"),sep=";",dec=",")

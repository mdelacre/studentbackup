###################################################################################
# POWER : S1 = Nskewed(0,2,skewness=1.99) ; S2 = Nskewed(1,2*ratio,skewness=1.99) #
###################################################################################
...
setwd(dir="C:/Users/mdela/Dropbox/Welch vs. Students_final Scripts/Table A3 power for distributions/Medium to large SS/Normal Skewed_ equal asymetry")
...
###### n=10 and r=0.5
	...
	powera_bis=function(ratio){
		...
		generate=function(i){
			...
			sample2 <- rsnorm(r*n1, mean=1, sd=ratio*2,xi=1.99)#simulate participants condition B, mean = 1
		...
		power_yuen <- sum(p_values[,5]<.05)/nSims
		return(c(ratio,n1,r,power_welch,power_student,power_yuen))

	}

	powera_rate=t(sapply(ratio,powera_bis))
	write.table(powera_rate,paste("Equal skewnorm_n1=",powera_rate[1,2],"and r=",powera_rate[1,3],".txt"),sep=";",dec=",")
